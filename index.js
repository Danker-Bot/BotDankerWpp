...
client.on('qr', qr => {
   ...
   console.log(`[ ${time} ] QR code is ready, subscribe danker ID`)
})
...
// client.on('user-presence-update', json => console.log(json.id + ' presence is => ' + json.type)) || console.log(`${time}: Bot by ig:@dankerigeno`)

client.on('message-status-update', json => {
   const participant = json.participant ? ' (' + json.participant + ')' : ''
   console.log(`[ ${time} ] => bot by ig:@dankerigeno`)
})
   ...
   switch (prefix) {
      ...
      case 'info':
       ...
       case 'nulis':
           nulis(value)
               .then(data => {
                   client.sendMessage(id, '[AGUARDE UN MOMENTO...', MessageType.text)
       ...
       case 'ytmp3':
           ytdl('mp3', value)
               .then(data => {
                   ...
                   let hasil = `AQUI ESTA EL LINK DE DESCARGA DE MSC\nCLICK EN EL LINK DE ABAJO:\nMUSICA: ${judul}\n\nTamaño Del audio: ${size}\n\nLink: ${link}`
       ...
       case 'twt':
           tweetdl(value)
               .then(data => {
                    ...
                    client.sendMessage(id, '[Aguarden un momento...', MessageType.text) 
                    client.sendMessage(id, hasil ,MessageType.text)
               })
               .catch(err => {
                   console.log(err)
       ...
       case 'tiktok':
           tiktokdl(value)
               .then(data => {
                    ...
                    let hasil = `✄1�71 ￄ 1/77 ¡¡¡Éxito !!! Haga clic en el enlace de abajo para descargar los resultados! \nClique en el link de abajo🗡️\n\nTítulo: ${deskripsi} \n\nDurasi: ${durasi}\n\nNome: ${nama}\n\nUrl: ${url}`;
                    client.sendMessage(id, hasil, MessageType.text)
               })
               .catch(err => {
                   console.log(err)
       ...
       case 'wiki':
           wiki(value)
               .then(data => {
                    ...
                    let hasil = `📝De acuerdo con Wikipedia:\n\n${res}`
                    client.sendMessage(id, hasil, MessageType.text)
               })
               .catch(err => {
                   console.log(err)
       ...
       case 'covid':
           corona()
               .then(data => {
                   ...
                   let hasil = `📌ÚLTIMOS DATOS DEL DISTRITO COVID-19 DE ARGENTINA\n\n📍Positivo ==> ${positif} \n📍Curado ==> ${sembuh} \n📍Ripeadisimos ==> ${meninggal}`
                   client.sendMessage(id, hasil, MessageType.text)
               })
               .catch(err => {
                   console.log(err)
   ...
   case 'fb':
           fb(value)
               .then(data => {
                   ...
                   let hasil = `Elija la resolución que desee😙 \n\n\n HD ${resultHD} \n\n\n SD ${resultSD}`
                   client.sendMessage(id, hasil, MessageType.text)
               })
               .catch(err => {
                   console.log(err)
       ...
       case 'ocr':
           ...
           readTextInImage(media)
               .then(data => {
                   client.sendMessage(id, `*que onda amigo el texto de la imagen esta aqui.* \n\nResultado:: \n\n${data}`, MessageType.text);
               })
               .catch(err => {
                   console.log(err)
               })
       ...
       case 'igstalk':
           igStalk(value)
               .then(data => {
                   ...
                   let hasil = `✨Bio de Instagram _${value}_ \n\n 🧶 *Nombre de usuário* : ${Username}_ \n 🌀 *Nome* : _${Name}_ \n 🌟 *Número de Seguidores* : _${Jumlah_Followers}_ \n 🌠 *Total_seguidores* : _${Jumlah_Following}_ \n ⭄1�71ￄ1�77 *Posteos_totales* : _${Jumlah_Post}_ `
                   client.sendMessage(id, hasil, MessageType.text)
               })
               .catch(err => {
...
case 'puisi2':
 ...
 case 'infogempa':
   ...
   case 'chord':
           chord(value)
               .then(data => {
                   const { result } = data
                   let hasil = `Aqui estan los acordes de la canción *${value}* querida ♥️\n\n  _${result}_ `
                   client.sendMessage(id, hasil, MessageType.text)
               })
               .catch(err => {
                   console.log(err)
 ...
 case 'wikien':
           wikien(value)
               .then(data => {
                   const { result } = data
                   let hasil = `*♻️De acuerdo con Wikipedia🗿:* \n\n  _${result}_ `
                   client.sendMessage(id, hasil, MessageType.text)
               })
               .catch(err => {
                   console.log(err)
      ...
      case 'stickergif':
            if (isMedia) {
                if (mimetype === 'video/mp4' && message.duration < 10 || mimetype === 'image/gif' && message.duration < 10) {
                    ...
                    client.reply(from, '[❗] Envié un video poniendo *!stickerGif* max 10 seg!', id)
                )
            }
            break     
     case 'sticker':
